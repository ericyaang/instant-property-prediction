...
def get_urls(link, pages):
    ...
    for page in range(1, pages + 1):
        ...
        # request
        olxRequest = requests.get(
            "{}?o={}".format(link, page), headers={"User-Agent": "Mozilla/5.0"}
        )
        try:
            ...
            print(
                "Status code error {} in page {}".format(olxRequest.status_code, page)
            )
            errors.append(page)
            continue
...
def get_infos(codigo, descricao, link, page, region):
    ...
    # time
    time = (
        soup.find("div", {"class": "sc-hmzhuo fpFNoN sc-jTzLTM iwtnNi"})
        .find("span")
        .get_text()
    )
    infos["Data"] = time
...
def get_urls_df(region, pages):
    ...
    for page in pages:
        ...
        temp = pd.DataFrame(pages.get(page), columns=["id", "description", "link"])
        temp["page"] = page
        temp["region"] = region
